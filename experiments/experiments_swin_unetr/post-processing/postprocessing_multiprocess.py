...
def run(data):
    ...
    transforms = monai.transforms.Compose([
        ...
        monai.transforms.CropForegroundd(keys=['pred', 'gt'], source_key='pred', margin=[100, 100, 100]),
    ])
...
if __name__ ==  '__main__':
    ...
    with open("/data_analysis/accepted_names.pkl", "rb") as fp:   # Unpickling
        accepted_names = pickle.load(fp)

    inference_path = "/swin_unetr/inference"
    for fold in range(0, 5):
        print(f'Starting fold {fold}.')
        min_gt_volume = {}
        for key, value in enumerate(np.load(f"/nnUNet_inference/organ_volumes_per_fold/organ_volumes_fold_{fold}.npy")): # We use the same organ volumes as nnUNet
        ...
        if args.pp_config in [1, 2]:
            np.save(f'/experiments_Swin-UNETR/post-processing/results/pp{args.pp_config}/rate_{args.rate}/fold_{fold}.npy', np.array(result))
        elif args.pp_config in [3, 4]:
            np.save(f'/experiments_Swin-UNETR/post-processing/results/pp{args.pp_config}/fold_{fold}.npy', np.array(result))
        else:
            np.save(f'/experiments_Swin-UNETR/post-processing/results/no_pp/fold_{fold}.npy', np.array(result))